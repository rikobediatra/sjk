...
/**
 * The Travel_Joy_Customizer_Modules_CSS object.
 */
class Travel_Joy_Customizer_Modules_CSS {
	...
	protected function __construct() {
		$class_files = array(
			'Travel_Joy_Customizer_Modules_CSS_Generator'               => '/class-kirki-modules-css-generator.php',
			'Travel_Joy_Customizer_Output'                              => '/class-kirki-output.php',
			'Travel_Joy_Customizer_Output_Field_Background'             => '/field/class-kirki-output-field-background.php',
			'Travel_Joy_Customizer_Output_Field_Image'                  => '/field/class-kirki-output-field-image.php',
			'Travel_Joy_Customizer_Output_Field_Multicolor'             => '/field/class-kirki-output-field-multicolor.php',
			'Travel_Joy_Customizer_Output_Field_Dimensions'             => '/field/class-kirki-output-field-dimensions.php',
			'Travel_Joy_Customizer_Output_Field_Typography'             => '/field/class-kirki-output-field-typography.php',
			'Travel_Joy_Customizer_Output_Property'                     => '/property/class-kirki-output-property.php',
			'Travel_Joy_Customizer_Output_Property_Background_Image'    => '/property/class-kirki-output-property-background-image.php',
			'Travel_Joy_Customizer_Output_Property_Background_Position' => '/property/class-kirki-output-property-background-position.php',
			'Travel_Joy_Customizer_Output_Property_Font_Family'         => '/property/class-kirki-output-property-font-family.php',
		);
	...
	public function init() {
		...
		Travel_Joy_Customizer_Modules_Webfonts::get_instance();
	...
	public function print_styles() {
		...
		$configs = Travel_Joy_Customizer::$config;
	...
	public static function loop_controls( $config_id ) {
		...
		// Get an instance of the Travel_Joy_Customizer_Modules_CSS_Generator class.
		// This will make sure google fonts and backup fonts are loaded.
		Travel_Joy_Customizer_Modules_CSS_Generator::get_instance();
		...
		foreach ( $fields as $field ) {
				...
				if ( ! empty( $field['required'] ) ) {
					...
					foreach ( $field['required'] as $requirement ) {
						if ( isset( $requirement['setting'] ) && isset( $requirement['value'] ) && isset( $requirement['operator'] ) ) {
							$controller_value = Travel_Joy_Customizer_Values::get_value( $config_id, $requirement['setting'] );
							if ( ! Travel_Joy_Customizer_Helper::compare_values( $controller_value, $requirement['value'], $requirement['operator'] ) ) {
								$valid = false;
							}
						}
					}
			...
			if ( isset( $field['output'] ) && ! empty( $field['output'] ) ) {
				$css = Travel_Joy_Customizer_Helper::array_replace_recursive( $css, Travel_Joy_Customizer_Modules_CSS_Generator::css( $field ) );

				// Add the globals.
				if ( isset( self::$css_array[ $config_id ] ) && ! empty( self::$css_array[ $config_id ] ) ) {
					Travel_Joy_Customizer_Helper::array_replace_recursive( $css, self::$css_array[ $config_id ] );
				}
			}
		}
		...
		if ( is_array( $css ) ) {
			return Travel_Joy_Customizer_Modules_CSS_Generator::styles_parse( Travel_Joy_Customizer_Modules_CSS_Generator::add_prefixes( $css ) );
		}
	}
