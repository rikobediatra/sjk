 ...
 */
class Travel_Joy_Customizer extends Travel_Joy_Customizer_Init {
	...
	public static function get_option( $config_id = '', $field_id = '' ) {
		return Travel_Joy_Customizer_Values::get_value( $config_id, $field_id );
	}
	...
	public static function add_config( $config_id, $args = array() ) {
		$config                             = Travel_Joy_Customizer_Config::get_instance( $config_id, $args );
	...
	public static function add_field( $config_id, $args ) {
		if ( doing_action( 'customize_register' ) ) {
			_doing_it_wrong( __METHOD__, esc_html__( 'Kirki fields should not be added on customize_register. Please add them directly, or on init.', 'travel-joy' ), '3.0.10' );
		}
		...
		$classname = 'Travel_Joy_Customizer_Field_' . str_replace( ' ', '_', ucwords( $str ) );
		...
		if ( false !== strpos( $classname, 'Travel_Joy_Customizer_Field_Kirki_' ) ) {
			$classname = str_replace( 'Travel_Joy_Customizer_Field_Kirki_', 'Travel_Joy_Customizer_Field_', $classname );
			if ( class_exists( $classname ) ) {
				new $classname( $config_id, $args );
				return;
			}
		}
		new Travel_Joy_Customizer_Field( $config_id, $args );
	}
