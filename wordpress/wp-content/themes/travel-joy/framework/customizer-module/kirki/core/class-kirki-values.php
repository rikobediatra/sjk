 ...
 */
class Travel_Joy_Customizer_Values {
	...
	public function typography_field_tweaks( $value, $field_id ) {
		if ( isset( Travel_Joy_Customizer::$fields[ $field_id ] ) && isset( Travel_Joy_Customizer::$fields[ $field_id ]['type'] ) ) {
			if ( 'kirki-typography' === Travel_Joy_Customizer::$fields[ $field_id ]['type'] ) {

				// Sanitize the value.
				// This also adds font-weight if it doesn't already exist.
				$value = Travel_Joy_Customizer_Field_Typography::sanitize( $value );
	 ...
	 * @static
	 * @access public
	 * @param string $config_id The configuration ID. @see Travel_Joy_Customizer_Config.
	 * @param string $field_id  The field ID.
	 * @return string|array
	 */
	public static function get_value( $config_id = '', $field_id = '' ) {
		...
		if ( ! isset( Travel_Joy_Customizer::$config[ $config_id ] ) ) {
			$config_id = 'global';
		}

		if ( 'theme_mod' === Travel_Joy_Customizer::$config[ $config_id ]['option_type'] ) {

			// We're using theme_mods so just get the value using get_theme_mod.
			$default_value = null;
			if ( isset( Travel_Joy_Customizer::$fields[ $field_id ] ) && isset( Travel_Joy_Customizer::$fields[ $field_id ]['default'] ) ) {
				$default_value = Travel_Joy_Customizer::$fields[ $field_id ]['default'];
			}
			$value = get_theme_mod( $field_id, $default_value );
			return apply_filters( 'kirki_values_get_value', $value, $field_id );
		}

		if ( 'option' === Travel_Joy_Customizer::$config[ $config_id ]['option_type'] ) {

			// We're using options.
			if ( '' !== Travel_Joy_Customizer::$config[ $config_id ]['option_name'] ) {

				// Options are serialized as a single option in the db.
				// We'll have to get the option and then get the item from the array.
				$options = get_option( Travel_Joy_Customizer::$config[ $config_id ]['option_name'] );

				if ( ! isset( Travel_Joy_Customizer::$fields[ $field_id ] ) && isset( Travel_Joy_Customizer::$fields[ Travel_Joy_Customizer::$config[ $config_id ]['option_name'] . '[' . $field_id . ']' ] ) ) {
					$field_id = Travel_Joy_Customizer::$config[ $config_id ]['option_name'] . '[' . $field_id . ']';
				}
				$setting_modified = str_replace( ']', '', str_replace( Travel_Joy_Customizer::$config[ $config_id ]['option_name'] . '[', '', $field_id ) );

				$default_value = ( isset( Travel_Joy_Customizer::$fields[ $field_id ] ) && isset( Travel_Joy_Customizer::$fields[ $field_id ]['default'] ) ) ? Travel_Joy_Customizer::$fields[ $field_id ]['default'] : '';
			...
			$value = get_option( $field_id, Travel_Joy_Customizer::$fields[ $field_id ]['default'] );
			return apply_filters( 'kirki_values_get_value', $value, $field_id );
