 ...
 * @since 3.0.10
 */
class Travel_Joy_Customizer_Autoload {
	...
	protected function autoload( $class_name ) {
		...
		if ( 0 !== stripos( $class_name, 'Travel_Joy_Customizer' ) ) {
			return;
		}
		 ...
		 */
		$class_name = travel_joy_customizer_kirki_autoloader_helper( 'Travel_Joy_Customizer', 'Kirki', $class_name );
	...
	protected function get_paths( $class_name ) {
		...
		if ( isset( $name_parts[0] ) && 'Modules' === $name_parts[0] ) {
			...
			$path .= strtolower( str_replace( '_', '-', travel_joy_customizer_kirki_autoloader_helper( 'Kirki_Modules_', '', $class_name ) ) ) . '/';

			$paths[] = $path . $filename;
		} elseif ( 'Travel_Joy_Customizer_Fonts' === $class_name ) {
